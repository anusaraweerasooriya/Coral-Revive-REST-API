...
class KGCN(object):
    ...
    def _build_model(self, n_user, n_entity, n_relation):
        with tf.compat.v1.variable_scope('KGCN', reuse=tf.compat.v1.AUTO_REUSE):
            self.user_emb_matrix = tf.compat.v1.get_variable(
                name='user_emb_matrix', shape=[n_user, self.dim], initializer=KGCN.get_initializer())
            self.entity_emb_matrix = tf.compat.v1.get_variable(
                name='entity_emb_matrix', shape=[n_entity, self.dim], initializer=KGCN.get_initializer())
    ...
    def _build_train(self):
        with tf.compat.v1.variable_scope('KGCN_train', reuse=tf.compat.v1.AUTO_REUSE):
